 ...
 */
class RangeFolder(filePath: String, workerPid: ActorRef, owner: ActorRef, range: KeyRange)
    extends PlainRpc {
  ...
  private def doRangeFold2(
      reader: RandomReader,
      workderPid: ActorRef,
      selfOrRef: ActorRef,
      range: KeyRange): Unit = {
    log.debug(
      "doRangeFold2, reader: %s, workerPid: %s, selfOrRef: %s, range: %s"
        .format(reader, workerPid, selfOrRef, range))
    val (_, valueList) = reader.rangeFold(
      (keyValue, acc0) => {
        (keyValue, acc0) match {
          ...
          case (e: KeyValue, (f, acc)) =>
            ...
            (f - 1, e :: acc)
          case unexpected =>
            throw new IllegalStateException("Unexpected value: %s".format(unexpected))
        }
      },
  ...
  def send(workerPid: ActorRef, selfOrRef: ActorRef, reverseKvs: List[Element]): Unit = {
    log.debug(
      "send, workerPid: %s, selfOrRef: %s, reverseKvs.size: %d"
        .format(workerPid, selfOrRef, reverseKvs.size))
